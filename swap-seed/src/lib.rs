...
use libp2p::identity;
use monero_seed::Seed as MoneroSeed;
use pem::{encode, Pem};
use rand::prelude::*;
use std::ffi::OsStr;
use std::fmt;
...
impl Seed {
    ...
    pub fn from_monero_seed(monero_seed: MoneroSeed) -> Self {
    ...
    pub fn from_file<D>(seed_file: D) -> Result<Self, Error>
    where
        D: AsRef<OsStr>,
    {
    ...
    pub fn write_to(&self, seed_file: PathBuf) -> Result<(), Error> {
        ensure_directory_exists(&seed_file).map_err(|e| Error::SwapFs(e.to_string()))?;
