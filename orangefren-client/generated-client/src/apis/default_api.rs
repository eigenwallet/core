...
pub async fn api_eigenwallet_create_path_get(configuration: &configuration::Configuration, create_path_request: models::CreatePathRequest) -> Result<models::CreatePathResponse, Error<ApiEigenwalletCreatePathGetError>> {
...
pub async fn api_eigenwallet_create_path_post(configuration: &configuration::Configuration, create_path_request: models::CreatePathRequest) -> Result<models::CreatePathResponse, Error<ApiEigenwalletCreatePathPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_create_path_request = create_path_request;
...
pub async fn api_eigenwallet_get_path_path_uuid_get(configuration: &configuration::Configuration, path_uuid: &str) -> Result<models::PathResponse, Error<ApiEigenwalletGetPathPathUuidGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_path_uuid = path_uuid;

    let uri_str = format!("{}/api/eigenwallet_get_path/{path_uuid}", configuration.base_path, path_uuid=crate::apis::urlencode(p_path_path_uuid));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiEigenwalletGetPathPathUuidGetError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn api_eigenwallet_get_path_path_uuid_post(configuration: &configuration::Configuration, path_uuid: &str) -> Result<models::PathResponse, Error<ApiEigenwalletGetPathPathUuidPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_path_uuid = path_uuid;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ApiEigenwalletGetPathPathUuidPostError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
