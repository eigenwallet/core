...
pub use ::monero_amount::{PrivateKey, PublicKey};
...
impl TransferRequest {
    pub fn address_and_amount(&self, network: Network) -> (MoneroAddress, ::monero_amount::Amount) {
        (
            MoneroAddress::new(
                network,
...
impl From<::monero_amount::Amount> for Amount {
    fn from(from: ::monero_amount::Amount) -> Self {
        Amount::from_piconero(from.as_pico())
    }
}

impl From<Amount> for ::monero_amount::Amount {
    fn from(from: Amount) -> Self {
        ::monero_amount::Amount::from_pico(from.as_piconero())
    }
}
...
mod tests {
    ...
    pub struct MoneroPrivateKey(
        #[serde(with = "swap_serde::monero::private_key")] ::monero_amount::PrivateKey,
    );

    #[derive(Debug, Serialize, Deserialize, PartialEq, Eq)]
    pub struct MoneroAmount(#[serde(with = "swap_serde::monero::amount")] ::monero_amount::Amount);

    #[test]
    fn serde_monero_private_key_json() {
        let key = MoneroPrivateKey(::monero_amount::PrivateKey::from_scalar(Scalar::random(
            &mut OsRng,
        )));
    ...
    fn serde_monero_amount() {
        let amount = MoneroAmount(::monero_amount::Amount::from_pico(1000));
