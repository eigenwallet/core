...
impl Rate {
    ...
    pub fn sell_quote(&self, quote: bitcoin::Amount) -> Result<monero_amount::Amount> {
        Self::quote(self.ask()?, quote)
    }

    fn quote(rate: bitcoin::Amount, quote: bitcoin::Amount) -> Result<monero_amount::Amount> {
        ...
        let base_in_piconero =
            base_in_xmr * Decimal::from(monero_amount::Amount::ONE_XMR.as_pico());

        let base_in_piconero = base_in_piconero
            .to_u64()
            .context("Failed to fit piconero amount into a u64")?;

        Ok(monero_amount::Amount::from_pico(base_in_piconero))
    }
}
...
mod tests {
    ...
    fn sell_quote() {
        ...
        assert_eq!(xmr_amount, monero_amount::Amount::ONE_XMR * 1000)
    }
