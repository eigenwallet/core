...
pub struct State0 {
    ...
    b: bitcoin::SecretKey,
    s_b: monero::Scalar,
    S_b_monero: monero_amount::PublicKey,
...
pub struct State1 {
    ...
    b: bitcoin::SecretKey,
    s_b: monero::Scalar,
    S_a_monero: monero_amount::PublicKey,
...
impl State3 {
    pub fn xmr_view_keys(&self) -> (monero_amount::PublicKey, monero::PrivateViewKey) {
        let S_b_monero = monero_amount::PublicKey::from_private_key(
            &monero_amount::PrivateKey::from_scalar(self.s_b),
        );
...
impl State4 {
    pub async fn check_for_tx_redeem(
        ...
        if let Some(tx_redeem_candidate) = tx_redeem_candidate {
            ...
            let s_a = monero_amount::PrivateKey::from_scalar(
...
impl State5 {
    pub fn xmr_keys(&self) -> (monero_amount::PrivateKey, monero::PrivateViewKey) {
        let s_b = monero_amount::PrivateKey { scalar: self.s_b };
        let s = self.s_a + s_b;
...
impl State6 {
    ...
    pub fn attempt_cooperative_redeem(
        ...
        let s_a = monero_amount::PrivateKey::from_scalar(s_a);
