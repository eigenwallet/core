...
impl State3 {
    ...
    pub fn lock_xmr_transfer_request(&self) -> TransferRequest {
        let S_a = monero_amount::PublicKey::from_private_key(&monero_amount::PrivateKey {
            scalar: self.s_a,
        });
    ...
    ) -> Result<monero_amount::PrivateKey> {
        Ok(monero_amount::PrivateKey::from_scalar(
            self.tx_refund()
    ...
    pub async fn refund_btc(
        &self,
        bitcoin_wallet: &dyn bitcoin_wallet::BitcoinWallet,
    ) -> Result<Option<monero_amount::PrivateKey>> {
