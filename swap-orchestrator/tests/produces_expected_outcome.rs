...
async fn test_config_generates_expected_compose() {
    ...
    let test_config_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/output/config.toml");

    let config = swap_env::config::Config::read(&test_config_path)
        .expect("Failed to read test config.toml");

    // 2. Determine if we should create full nodes based on the config
    let create_full_bitcoin_node = swap_orchestrator::util::should_create_full_bitcoin_node(&config);
    ...
    let compose_name = swap_orchestrator::util::compose_name(
    ...
    let expected_yaml_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/output/docker-compose.yml");
    ...
    let generated_value: serde_yaml::Value = serde_yaml::from_str(&generated_yaml)
        .expect("Failed to parse generated YAML");
    let expected_value: serde_yaml::Value = serde_yaml::from_str(&expected_yaml)
