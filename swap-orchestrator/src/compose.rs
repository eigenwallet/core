...
fn build(input: OrchestratorInput) -> String {
    ...
    if input.want_killswitch {
        command_asb
            .0
            .splice(0..0, [flag!("killswitch"), flag!("supervise"), flag!("--")]);
    }
...
",
    ...
    let (killswitch_segment, killswitch_volume, asb_killswitch_volume) = if input.want_killswitch {
        let command_killswitch = command!["killswitch", flag!("monitor")];
        let segment = format!(
            "\
  killswitch:
    ...
    logging: *default-logging
    volumes:
      - 'killswitch-state:/run/killswitch'
    healthcheck:
      test: [\"CMD\", \"killswitch\", \"health\"]
      interval: 10s
      timeout: 5s
      retries: 1
      start_period: 5s
    entrypoint: ''
    command: {command_killswitch}\
",
            image_killswitch = input.images.killswitch.to_image_attribute(),
        );
        (
            segment,
            "killswitch-state:",
            "\n      - 'killswitch-state:/run/killswitch:ro'",
        )
    } else {
        (String::new(), "", "")
    };
...
",
    ...
    let asb_depends_on = if input.want_killswitch {
        "      electrs:
...
{asb_depends_on}
    volumes:
      ...
      - 'asb-data:{asb_data_dir}'{asb_killswitch_volume}
    ports:
