...
pub async fn punish(
    ...
    let (state3, transfer_proofs) = match state {
        ...
        AliceState::XmrLockTransactionSent {state3, transfer_proof: transfer_proofs, ..}
        | AliceState::XmrLocked {state3, transfer_proof: transfer_proofs, ..}
        | AliceState::XmrLockTransferProofSent {state3, transfer_proof: transfer_proofs, ..}
        | AliceState::EncSigLearned {state3, transfer_proof: transfer_proofs, ..}
        | AliceState::CancelTimelockExpired {state3, transfer_proof: transfer_proofs, ..}
        // Punish possible due to cancel transaction already being published
        | AliceState::BtcCancelled {state3, transfer_proof: transfer_proofs, ..}
        | AliceState::BtcPunishable {state3, transfer_proof: transfer_proofs, ..}
        ...
        | AliceState::BtcRedeemTransactionPublished { state3, transfer_proof: transfer_proofs, .. }
        | AliceState::BtcRefunded { state3, transfer_proof: transfer_proofs,.. } => { (state3, transfer_proofs) }
