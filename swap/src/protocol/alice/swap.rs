...
fn build_transfer_destinations(
    ...
    lock_amount: monero_amount::Amount,
    tip: TipConfig,
) -> anyhow::Result<Vec<(monero_address::MoneroAddress, monero_amount::Amount)>> {
    ...
    if tip_amount_piconero >= MIN_USEFUL_TIP_AMOUNT_PICONERO {
        let tip_amount = monero_amount::Amount::from_pico(tip_amount_piconero);
