...
pub struct GetMoneroAddressesResponse {
    ...
    pub addresses: Vec<MoneroAddressWrapper>,
}

#[derive(Serialize, Deserialize, Debug)]
#[serde(transparent)]
pub struct MoneroAddressWrapper(
    #[serde(with = "swap_serde::monero::address_serde")] monero_address::MoneroAddress,
);
...
impl Request for GetMoneroAddressesArgs {
    ...
    async fn request(self, ctx: Arc<Context>) -> Result<Self::Response> {
        ...
        Ok(GetMoneroAddressesResponse {
            addresses: addresses.into_iter().map(MoneroAddressWrapper).collect(),
        })
    }
}
...
impl Request for SendMoneroArgs {
    ...
    async fn request(self, ctx: Arc<Context>) -> Result<Self::Response> {
        ...
        let approval_callback: Arc<
            dyn Fn(String, monero_amount::Amount, monero_amount::Amount) -> BoxFuture<'static, bool>
                + Send
                + Sync,
        > = std::sync::Arc::new(
            move |_txid: String, amount: monero_amount::Amount, fee: monero_amount::Amount| {
...
pub async fn monero_recovery(
    ...
    if let BobState::BtcRedeemed(state5) = swap_state {
        ...
        let address = monero_address::MoneroAddress::new(
            ...
            monero_amount::PublicKey::from_private_key(&spend_key).decompress(),
