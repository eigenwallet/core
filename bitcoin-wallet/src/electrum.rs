...
use tokio::sync::{Mutex as TokioMutex, RwLock as TokioRwLock, watch};
...
/// Status-update channels we poll, keyed by the watched transaction and script.
type Subscriptions = Arc<TokioMutex<HashMap<(Txid, ScriptBuf), watch::Sender<ScriptStatus>>>>;
...
pub struct Client {
    ...
    pub(crate) subscription_idle_timeout: Duration,
    /// Monotonic latest known block height.
    latest_block_height: Arc<SyncMutex<BlockHeight>>,
}

/// Default duration a subscription without receivers is kept alive.
pub(crate) const DEFAULT_SUBSCRIPTION_IDLE_TIMEOUT: Duration = Duration::from_secs(4 * 60);
...
impl Client {
    ...
    async fn update_script_history_for(&self, script: ScriptBuf) -> Result<()> {
        ...
        if !any_success && let Some(e) = first_error {
    ...
    /// Fetch a transaction from the Electrum servers.
    ///
    ...
    pub async fn get_tx(&self, txid: Txid) -> Result<Option<Arc<Transaction>>> {
        let results = self
            .inner
            .join_quorum("get_raw_transaction", move |conn| async move {
                match conn.request(GetTx { txid }).await {
                    Ok(full) => Ok(Some(full.tx)),
                    // A recognized "not found" is a valid answer, not a server failure
                    Err(e) if is_tx_not_found(&e) => Ok(None),
                    Err(e) => Err(e),
                }
            })
            .await
        ...
        let required_not_found = self
            ...
            .clamp(1, self.inner.server_count());
        ...
        let all_servers_finished = results.len() >= self.inner.server_count();
...
impl EstimateFeeRate for Client {
    async fn estimate_feerate(&self, target_block: u32) -> Result<FeeRate> {
        ...
        match (conservative, histogram) {
            ...
            (Err(conservative_error), Err(histogram_error)) => {
                Err(conservative_error.context(histogram_error).context(
    ...
    async fn populate_with_outpoints(
        ...
        for op in outpoints {
            if let Ok(tx) = self.fetch_tx(op.txid).await
                && let Some(txout) = tx.output.get(op.vout as usize)
            {
