...
```mermaid
graph TD
    A["Swap starts"] --> B{{"Swap completes within 4h?"}}
...
_Situation: A swap was started, but was not completed within 4h._
...
This "cancel timelock" is currently set to 24 Bitcoin confirmations, or ~4 hours.
The cancel transaction does not return the Bitcoin to your wallet.
This will be handled in the next step.

## Refund

_Situation: A swap was started, but was not completed within 4h. The maker subsequently cancelled the swap by publishing the cancel transaction._
...
<Callout type="important">
  The refund window is 24h long and starts 4h after the swap was started.
</Callout>
...
_Situation: A swap was started, but was not completed within 4h. The maker subsequently cancelled the swap. But even after another 24h the taker still hasn't refunded._
