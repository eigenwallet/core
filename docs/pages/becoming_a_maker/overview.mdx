...
| `ask_spread` | The markup the asb will charge compared to the market price, as a factor. The market price is fetched via the `price_ticker_ws_url_kraken`, `price_ticker_ws_url_bitfinex`, and `price_ticker_rest_url_kucoin`. A value of `0.02` means the asb will charge 2% more than the market price. |
