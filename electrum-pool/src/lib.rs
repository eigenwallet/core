...
pub trait ConnectionFactory<C>: Send + Sync {
    ...
    ) -> BoxFuture<'static, Result<C, Error>>;
}
...
pub struct ElectrumBalancerConfig {
    ...
    /// Number of successful responses parallel multi-server operations
    /// ([`ElectrumBalancer::join_quorum`], [`ElectrumBalancer::broadcast_all`])
    /// wait for before returning early. Clamped to the number of servers.
...
pub struct ElectrumBalancer<C = Connection> {
    ...
    config: Arc<ElectrumBalancerConfig>,
    ...
    C: Send + Sync + 'static,
{
    ...
    where
        F: Fn(Arc<C>) -> Fut + Send + Sync + Clone + 'static,
        ...
        T: Send + 'static,
    {
        let tasks = (0..self.urls.len()).map(|idx| {
            ...
            tokio::spawn(async move {
                let result = match balancer.get_or_connect(idx).await {
                    Ok(connection) => f(connection).await,
                    Err(e) => Err(e),
                };
                if let Err(e) = &result
                    && e.is_connection()
                {
                    balancer.invalidate(idx).await;
                }
                result
            })
        });
        ...
        for task in tasks {
            match task.await {
                ...
                Err(err) => {
                    ...
                    results.push(Err(Error::connection(format!("Task failed: {err}"))));
                }
            }
        }
        results
    ...
    /// Run the operation against every server in parallel, returning as soon as
    /// `min_parallel_responses` servers have responded successfully and the primary (first) server
    /// has produced a result.
    ///
    /// The requests to the remaining servers keep running in the background; their results are
    /// discarded. If the quorum cannot be reached the call returns once every server has responded.
    ///
    /// The returned results are in completion order, not server order. The caller is responsible
    /// for judging whether the collected responses are sufficient evidence to act on.
    #[instrument(level = "debug", skip(self, f), fields(operation = kind, servers = self.urls.len(), min_parallel_responses = self.config.min_parallel_responses))]
    pub async fn join_quorum<F, Fut, T>(
        &self,
        kind: &str,
        f: F,
    ...
    where
        ...
        T: Send + 'static,
    {
        let quorum = self.config.min_parallel_responses.clamp(1, self.urls.len());

        let mut tasks: FuturesUnordered<_> = (0..self.urls.len())
            .map(|idx| {
                let f = f.clone();
                let balancer = self.clone();
        ...
        while let Some(joined) = tasks.next().await {
            match joined {
                ...
                Err(err) if err.is_cancelled() => {
                    ...
                    return Err(Error::connection("Task cancelled"));
                }
                Err(err) => {
                    trace!(error = ?err, "Failed to join task for parallel request");
                }
        ...
        trace!(
            ...
            collected_responses = results.len(),
...
impl ElectrumBalancer<Connection> {
    ...
    pub async fn request_join_all<Req>(
        &self,
        kind: &str,
        req: Req,
    ) -> Vec<Result<Req::Response, Error>>
    where
        Req: RequestExt + Clone + Send + Sync + 'static,
        Req::Response: Send,
    {
    ...
    /// Issue a typed request against every server concurrently, returning early once the
    /// `min_parallel_responses` quorum (and the primary server) has responded.
    ///
    /// Remaining requests keep running in the background; their results are discarded. If the
    /// quorum cannot be reached, this waits for every server and returns all collected results.
    pub async fn request_join_quorum<Req>(
        &self,
        kind: &str,
        req: Req,
    ) -> Result<Vec<Result<Req::Response, Error>>, Error>
    where
        Req: RequestExt + Clone + Send + Sync + 'static,
        Req::Response: Send,
    {
        self.join_quorum(kind, move |connection| {
            let req = req.clone();
            async move { connection.request(req).await }
        })
        .await
    ...
    /// Broadcast a transaction to every server concurrently, returning early once the
    /// `min_parallel_responses` quorum (and the primary server) has responded.
    pub async fn broadcast_all(
        &self,
        tx: Transaction,
    ) -> Result<Vec<Result<bitcoin::Txid, Error>>, Error> {
        self.request_join_quorum("transaction_broadcast", BroadcastTx(tx))
            .await
    }

    /// Fetch a single script's history from every server concurrently, returning early once the
    /// `min_parallel_responses` quorum (and the primary server) has responded.
    pub async fn script_get_history_all(
        &self,
        script: bitcoin::ScriptBuf,
    ) -> Result<Vec<Result<Vec<response::Tx>, Error>>, Error> {
        use electrum_streaming_client::request::GetHistory;
        self.request_join_quorum("script_get_history", GetHistory::from_script(script))
            .await
    }
}
...
mod tests {
    ...
    async fn sticky_stays_on_first_server() {
        ...
        for _ in 0..5 {
            let result = balancer
                .run("test", |c| async move { c.call().await })
                .await;
            assert!(result.is_ok());
        }
    ...
    async fn all_fail_yields_multi_error() {
        ...
        let balancer = ElectrumBalancer::new_with_factory(urls, fast_config(), factory).unwrap();
    ...
    async fn join_all_hits_every_server() {
        ...
        let results = balancer
            .join_all("test", |c| async move { c.call().await })
            .await;
