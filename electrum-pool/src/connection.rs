...
impl Connection {
    ...
    pub async fn connect(url: &str, request_timeout: Duration) -> Result<Self, Error> {
        ...
        let (client, worker) = if target.use_tls {
            ...
            let server_name = ServerName::try_from(target.host.clone()).map_err(|e| {
                Error::connection(format!("Invalid TLS server name for {url}: {e}"))
            })?;
        ...
        Req::Response: Send,
    {
        match tokio::time::timeout(self.request_timeout, self.client.send_request(req)).await {
            ...
            Ok(Err(AsyncRequestError::Dispatch(e))) => Err(Error::connection(format!(
                "Failed to dispatch request: {e}"
            ))),
...
fn spawn_client<R, W>(
    reader: R,
    writer: W,
) -> (AsyncClient, tokio::task::JoinHandle<std::io::Result<()>>)
where
