...
pub struct DocumentManager {
    documents: HashMap<DocumentId, AutoCommit>,
    actor_id: ActorId,
}
...
impl DocumentManager {
    ...
    pub fn get_or_create_document(&mut self, document_id: &DocumentId) -> Result<&mut AutoCommit> {
    ...
    pub fn get_latest_state(&self, document_id: &DocumentId) -> Result<Option<Value>> {
    ...
    /// Apply changes to a document using AutoCommit
    pub fn apply_changes(
        ...
        let doc = self.get_or_create_document(document_id)?;
        match doc.apply_changes(changes) {
            Ok(_) => Ok(()),
            Err(e) => Err(e.into()),
        }
    }
    ...
    /// Get document heads
    pub fn get_heads(&mut self, document_id: &DocumentId) -> Option<Vec<ChangeHash>> {
    ...
    pub fn serialize_document(&mut self, document_id: &DocumentId) -> Result<Option<Vec<u8>>> {
        if let Some(doc) = self.documents.get_mut(document_id) {
            let bytes = doc.save();
            Ok(Some(bytes))
        } else {
    ...
    pub fn load_document(&mut self, document_id: &DocumentId, data: &[u8]) -> Result<()> {
...
mod tests {
    ...
    fn test_get_or_create_document() {
        ...
        let _doc1 = manager.get_or_create_document(&document_id).unwrap();
        
        // Second call should return the existing document (test that no panic occurs)
        let _doc2 = manager.get_or_create_document(&document_id).unwrap();
        
        // Document should exist in the manager
        assert!(manager.documents.contains_key(&document_id));
    }
} 